...
b8 engine_run(void) {
    ...
    while (engine->is_running && !etwindow_should_close(engine->window)) {
        if (!engine->is_minimized) {
            ...
            printf("Frame time: %2.8llfms | x: %lf y: %lf z: %lf\r", dt * 1000, engine->main_scene->cam.position.x, engine->main_scene->cam.position.y, engine->main_scene->cam.position.z);
            clock_start(&engine->frame);
