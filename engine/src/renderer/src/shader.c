...
b8 load_shader(renderer_state* state, const char* path, shader* out_shader) {
    ...
    ETINFO("set_count before enumerate: %lu.", set_count);
    ...
    ETINFO("push_constant_count after enumerate: %lu.", push_constant_count);
    ...
    ETINFO("input_variable_count before enumerate: %lu.", input_variable_count);
    ...
    ETINFO("input_variable_count after enumerate: %lu.", input_variable_count);
    ...
    ETINFO("output_variable_count before enumerate: %lu.", output_variable_count);
    ...
    ETINFO("output_variable_count after enumerate: %lu.", output_variable_count);


    // Enumerate set count but only enumerate the bindings
    out_shader->reflection.set_count = set_count;
    out_shader->reflection._sets = etallocate(sizeof(set) * set_count, MEMORY_TAG_SHADER);
    for (u32 i = 0; i < set_count; ++i) {
        // Pointer for easier to read code
        set* curr_set = &out_shader->reflection._sets[i];
...
void unload_shader(renderer_state* state, shader* shader) {
    ...
    for (u32 i = 0; i < shader->reflection.set_count; ++i) {
        etfree(shader->reflection._sets[i]._bindings, sizeof(binding) * shader->reflection._sets[i].binding_count, MEMORY_TAG_SHADER);
    }
    etfree(shader->reflection._sets, sizeof(set) * shader->reflection.set_count, MEMORY_TAG_SHADER);
